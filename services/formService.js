  ...
  const promise = new Promise((resolve, reject) => {
    ...
    if (fathersFirstName && fathersLastName) {
      ...
      body.request.payload.contact.keyContact[0].caseContactName =
        fathersFirstName + " " + fathersLastName;
    } else {
    ...
    body.request.payload.work.companyId = companyId
    ...
    body.request.payload.address[1].pincode = officePincode
      ...
      : "";
    body.request.payload.address[1].city = officePincode
      ? officePincode.cityId
      : "";
    body.request.payload.address[1].state = officeNearBy
      ? officePincode.stateId
      : "";
...
export const getProductAndBank = async (data, primaryPath, longFormProduct) => {
  let productData = await strapi.processReq(
    "GET",
    `credit_card_product?slug=${longFormProduct}`
  );
};
