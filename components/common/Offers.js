...
import { viewOffer, customerOfferData, getProductDecision } from '../../services/offersService'
...
const Offers = props => {
   ...
   useEffect(() => {
      ...
      // }, 2000);
      setCards(props.data.product_v_2s)
      dataToShow()
   }, [])
      ...
      <section data-aos="fade-up" className="container popular-card-container">
         <div className="popular-cards">
            ...
            <div className="popular-cards-slider" id="popular-cards-sec">
               {cards.map(offer => {
                  const unpackedOffer = unpackComponents(offer)
                  const { bank, product } = unpackedOffer
                  const { product_name, product_feature, product_annual_fee, product_usp_highlight } = product
                  return (
                     <div className="popular-cards-slider-card" key={product.id} onClick={() => redirectToDetailsPage(unpackedOffer)}>
