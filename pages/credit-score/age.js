...
const utilization = (props) => {
  console.log({ props })
  const active = props?.data?.creditAge?.filter((item) => item.accountStatus === 'ACTIVE')
  const closed = props?.data?.creditAge?.filter((item) => item.accountStatus !== 'ACTIVE')
  return (
...
export async function getServerSideProps(ctx) {
  ...
  const responseObject = await fetch('http://203.122.46.189:8060/utility/api/credit-profile/v1/credit-age', {
    method: 'POST',
    headers: {
